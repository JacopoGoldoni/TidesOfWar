...
public static class UtilityMath
{
    ...
    //GRID
    public static Vector2[,] PointsGrind(int sizeX, int sizeY, float deltaX, float deltaY, float offSetX, float offSetY)
    {
        Vector2[,] points = new Vector2[sizeX,sizeY];
        
        for(int i = 0; i < sizeX; i++)
        {
            for (int j = 0; j < sizeY; j++)
            {
                points[i, j] = new Vector2(i * deltaX + offSetX, j * deltaY + offSetY);
            }
        }
        
        return points;
    }
    public static Vector2[,] PointsGrindRnd(int sizeX, int sizeY, float deltaX, float deltaY, float offSetX, float offSetY, float rndRadius)
    {
        Vector2[,] points = new Vector2[sizeX, sizeY];
        ...
        for (int i = 0; i < sizeX; i++)
        {
            for (int j = 0; j < sizeY; j++)
            {
                Vector2 rnd = RandomPointInDisc(rndRadius);
                points[i, j] = new Vector2(i * deltaX + offSetX, j * deltaY + offSetY) + rnd;
            }
        }
